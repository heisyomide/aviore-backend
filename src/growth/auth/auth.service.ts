...
export class GrowthAuthService {
  ...
  async validateTeamMember(loginDto: LoginDto): Promise<{ accessToken: string }> {
    ...
    // 3. Encode matching fields into payload to make tracking foolproof inside downstream handlers
    const tokenPayload = {
      sub: authenticatedMember.id, // Industry standard JWT claim definition
      id: authenticatedMember.id,  // Direct object structural mapping shortcut
