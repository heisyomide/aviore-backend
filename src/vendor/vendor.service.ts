...
import { OrderStatus, Prisma, ProductStatus, VoucherStatus } from '@prisma/client';
...
export class VendorService {
   ...
   */
  async createProduct(vendorId: string, dto: VendorCreateProductDto, file: Express.Multer.File) {
    if (!vendorId) {
      throw new BadRequestException('Vendor ID tracking context is required.');
    }
    
    const vendor = await this.prisma.vendor.findUnique({ where: { id: vendorId } });
    if (!vendor) {
      throw new NotFoundException(`Vendor registry node [${vendorId}] not found.`);
    }

    // Actual creation mapping using your DTO properties
    try {
      this.logger.log(`Product creation routine hit for vendor: ${vendorId}`);
      
      // Configure default Cloudinary configurations if file is present
      let uploadedImageUrl = null;
      if (file) {
        // Implement upload stream logic matching your secure KYC structure if necessary
      }
      ...
      const product = await this.prisma.product.create({
        data: {
          ...
          stock: dto.stock ?? 0,
          status: ProductStatus.PENDING, // Default state until admin approval
          categoryId: dto.categoryId,
        }
      });

      return { success: true, message: 'Product creation mechanism finalized.', data: product };
    } catch (error) {
      this.logger.error(`Product creation failed: ${error instanceof Error ? error.message : error}`);
      throw new InternalServerErrorException('Failed to complete product catalog generation.');
    }
  }
  ...
  async getVendorDashboard(vendorId: string) {
    ...
    return {
      ...
      recentOrders: recentOrders.map((item) => ({
        id: item.orderId,
        artifact: item.product?.title ?? 'Product',
      ...
      const request = await tx.withdrawalRequest.create({
        data: {
          ...
          metadata: {
            ...
            vendorEmail: vendor.user?.email || null,
          },
        },
      });
  ...
  async getVendorTickets(userId: string) {
  ...
  // FIXED: Now accurately expects validated vendorId directly from controller
  ...
  // FIXED: Cleared intermediate user lookup loop complexity
  async getConversationById(conversationId: string, vendorId: string) {
  ...
  async submitKyc(userId: string, idType: string, idNumber: string, file: Express.Multer.File) {
    ...
    try {
      ...
      throw new InternalServerErrorException(
        error instanceof Error ? error.message : 'An error occurred while transmitting your identity documents.',
      );
    }
  }
  ...
  async markOrderAsCompleted(orderId: string, vendorId: string) {
    return await this.prisma.$transaction(async (tx) => {
      ...
      if (order.status === OrderStatus.COMPLETED) return { status: 'ALREADY_COMPLETED' };

      await tx.order.update({
        where: { id: orderId },
        data: { 
          status: OrderStatus.COMPLETED,
          totalPaid: order.totalPaid ? order.totalPaid : order.totalAmount
        },
      });
      ...
      for (const item of order.items) {
        ...
        const calculatedCommission = totalGross * 0.10;
        const calculatedEarning = totalGross - calculatedCommission;
        ...
        if (item.payoutStatus !== 'PAID' && totalGross > 0) {
          ...
          await tx.orderItem.update({
            where: { id: item.id },
            data: { 
              vendorEarning: calculatedEarning,
      ...
      this.logger.log(`💰 AUDIT_COMPLETE: Order ${orderId} finalized.`);
  ...
  async findPublicVendors(params: { isVerified?: boolean; limit?: number; search?: string }) {
    ...
    try {
      ...
      this.logger.error("VENDOR_QUERY_FAILURE", error);
      throw new InternalServerErrorException("Failed to retrieve public vendor registry.");
    }
  }
  ...
  async updateOrderStatus(
    orderId: string, 
    vendorId: string, 
    dto: { status: string; trackingNumber?: string; carrier?: string }
  ) {
    ...
    if (!vendorItems.length) {
      throw new ForbiddenException('Order not found or access denied: You do not own any items in this checkout registry.');
    }
  ...
  // FIXED: Expects explicitly filtered vendorId context from controller
  async getReturnRequests(vendorId: string) {
  ...
  async triggerReturnMediation(returnId: string, vendorId: string, reason: string) {
    ...
    return this.prisma.returnRequest.update({
      ...
      data: {
        ...
        adminDecision: `VENDOR_DISPUTE_SIGNAL: ${reason}`, 
        updatedAt: new Date(),
      },
    });
  }
